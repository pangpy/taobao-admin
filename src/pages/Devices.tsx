...
const Devices: React.FC = () => {
    ...
    client.on('message', (topic: string, payload: Buffer) => {
      try {
        ...
        setDevices(prev => ({
          ...prev,
          [deviceId]: {
            id: deviceId, name: `设备 ${deviceId}`, online: true,
  ...
  const reportSimSensor = () => {
    ...
    simClientRef.current.publish(`user/${USER_ID}/${simDeviceId}/sensor`, JSON.stringify({
      temperature: simTemp, humidity: simHum, light: simLight,
    }));
  ...
  const reportSimStatus = (client?: mqtt.MqttClient) => {
    ...
    if (!c) return;
    c.publish(`user/${USER_ID}/${simDeviceId}/status`, JSON.stringify({
      temperature: simTemp, humidity: simHum, light: simLight,
      relay1: simRelay1, relay2: simRelay2,
    }));
  };

  const sendCommand = (deviceId: string, relay: number, value: boolean) => {
    if (!clientRef.current) return;
    clientRef.current.publish(`user/${USER_ID}/${deviceId}/command`, JSON.stringify({
      action: 'relay', pin: relay, value: value ? 1 : 0,
    }));
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <Space>
          <Tag color={connected ? 'green' : 'red'}>MQTT {connected ? '已连接' : '未连接'}</Tag>
          <Button icon={<ToolOutlined />} onClick={() => { setSimulatorOpen(true); connectSimulator(); }}>模拟器</Button>
          <Button icon={<ReloadOutlined />} onClick={connectMqtt}>重连</Button>
        </Space>
      </div>
      ...
      <Card size="small" className="mb-4">
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex gap-2">
            <Button type={connectType === 'ws' ? 'primary' : 'default'} icon={<WifiOutlined />} onClick={() => setConnectType('ws')}>WebSocket (wss://)</Button>
            <Button type={connectType === 'tcp' ? 'primary' : 'default'} icon={<WifiOutlined />} onClick={() => setConnectType('tcp')}>TCP (tcp://)</Button>
          </div>
          {connectType === 'tcp' && (
            <Space>
          ...
          <Text type="secondary" className="text-xs">{connectType === 'ws' ? '浏览器直连，走 443 端口' : '直连 TCP，低功耗低延迟'}</Text>
        </div>
      </Card>
      ...
      {deviceList.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <div className="mt-2 text-xs text-gray-400">等待设备上报数据... 主题: user/{USER_ID}/+/status</div>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {deviceList.map((device) => (
            <Card key={device.id} title={
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-blue-50 rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-blue-600">{device.data?.temperature?.toFixed(1) ?? '--'}°C</div>
                  <div className="text-xs text-gray-500 mt-1">温度</div>
                </div>
                <div className="bg-cyan-50 rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-cyan-600">{device.data?.humidity?.toFixed(1) ?? '--'}%</div>
                  <div className="text-xs text-gray-500 mt-1">湿度</div>
                </div>
                <div className="bg-yellow-50 rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-yellow-600">{device.data?.light ?? '--'}</div>
              ...
              <div className="border-t pt-3">
                ...
                <div className="flex gap-4">
                  <div className="flex items-center gap-2">
                    <BulbOutlined className="text-lg" /><span className="text-sm">开关1</span>
                    <Switch checked={device.data?.relay1} onChange={(v) => sendCommand(device.id, 1, v)} />
                  </div>
                  <div className="flex items-center gap-2">
                    <ThunderboltOutlined className="text-lg" /><span className="text-sm">开关2</span>
      ...
      <Drawer title="🔧 设备模拟器" open={simulatorOpen} onClose={() => { setSimulatorOpen(false); disconnectSimulator(); }} width={400}>
        <div className="space-y-4">
          <div><div className="text-sm text-gray-500 mb-1">设备ID</div><Input value={simDeviceId} onChange={(e) => setSimDeviceId(e.target.value)} /></div>
          <div><div className="text-sm text-gray-500 mb-1">温度 °C</div><Input type="number" value={simTemp} onChange={(e) => setSimTemp(Number(e.target.value))} /></div>
          <div><div className="text-sm text-gray-500 mb-1">湿度 %</div><Input type="number" value={simHum} onChange={(e) => setSimHum(Number(e.target.value))} /></div>
          <div><div className="text-sm text-gray-500 mb-1">光照 lux</div><Input type="number" value={simLight} onChange={(e) => setSimLight(Number(e.target.value))} /></div>
          <Button type="primary" block onClick={reportSimSensor}>📤 上报传感器数据</Button>
          <div className="border-t pt-4 mt-4">
            <div className="text-sm font-medium mb-3">开关状态</div>
            <div className="flex justify-between items-center mb-2"><span>开关1</span><Switch checked={simRelay1} onChange={(v) => { setSimRelay1(v); reportSimStatus(); }} /></div>
            <div className="flex justify-between items-center"><span>开关2</span><Switch checked={simRelay2} onChange={(v) => { setSimRelay2(v); reportSimStatus(); }} /></div>
          </div>
        </div>
      </Drawer>
    </div>
