...
const WebSocketTest: React.FC = () => {
  ...
  const [incomingCallA, setIncomingCallA] = useState(false);
  const [callTypeA, setCallTypeA] = useState<'audio' | 'video'>('audio');
  ...
  const [incomingCallB, setIncomingCallB] = useState(false);
  const [callTypeB, setCallTypeB] = useState<'audio' | 'video'>('audio');
  ...
  useEffect(() => {
    if (localVideoRefB.current && localStreamRefB.current) localVideoRefB.current.srcObject = localStreamRefB.current;
  }, [inCallB]);
  useEffect(() => {
    if (remoteVideoRefA.current && remoteStreamRefA.current) remoteVideoRefA.current.srcObject = remoteStreamRefA.current;
  }, [remoteVideoA]);
  useEffect(() => {
        ...
        try {
          ...
          if (data.type === 'file') {
            const msg: Message = {
              ...
              timestamp: new Date(), fromUserId: data.from, toUserId: data.to,
              fileUrl: data.file_url, fileName: data.file_name,
              fileSize: data.file_size, fileMime: data.file_mime,
            };
          ...
          if (data.type === 'webrtc_reject') {
            ...
            if (isA) setInCallA(false);
            else setInCallB(false);
            return;
          }
  ...
  const startWebRTC = async (userKey: UserKey, withVideo: boolean, isCaller: boolean) => {
    try {
      ...
      if (isA) localStreamRefA.current = stream;
      ...
      pc.ontrack = (event) => {
        ...
        else remoteStreamRefB.current = remoteStream;
        if (remoteStream.getVideoTracks().length > 0) {
          if (isA) setRemoteVideoA(true);
          else setRemoteVideoB(true);
        }
        if (remoteStream.getAudioTracks().length > 0) {
          const audio = new Audio(); audio.srcObject = remoteStream; audio.autoplay = true;
        }
      };
  ...
  const rejectCall = (userKey: UserKey) => {
    ...
    else { setIncomingCallB(false); setInCallB(false); }
  ...
  const uploadFile = async (file: File, userKey: UserKey): Promise<{ url: string; filename: string; size: number; mime: string } | null> => {
    ...
    try {
      ...
      const response = await fetch('https://api.apiscode.org/api/upload', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${getUploadToken()}` },
        body: formData,
      });
      const result = await response.json();
      if (result.code === 200) return { url: result.data.url, filename: result.data.filename, size: result.data.size, mime: result.data.mime };
      else { message.error(result.message || '上传失败'); return null; }
    } catch { message.error('上传失败'); return null; }
  ...
  const sendFileMessage = (userKey: UserKey, fileInfo: { url: string; filename: string; size: number; mime: string }) => {
    ...
    try {
      wsRef.current.send(JSON.stringify({
        type: 'file', content: `[文件] ${fileInfo.filename}`,
        file_url: fileInfo.url, file_name: fileInfo.filename,
        file_size: fileInfo.size, file_mime: fileInfo.mime,
        to_user_id: targetUser.id, to_role: targetUser.role,
      }));
      setMessages(prev => [...prev, {
        id: Date.now().toString(), type: 'file',
        content: `[文件] ${fileInfo.filename}`, timestamp: new Date(),
        fromUserId: currentUser.id, toUserId: targetUser.id,
  ...
  const handleFileSelect = async (file: File, userKey: UserKey) => {
    if (file.size > 2 * 1024 * 1024) { message.error('文件大小不能超过2MB'); return false; }
    const hide = message.loading('正在上传...', 0);
    const result = await uploadFile(file, userKey);
    hide();
    if (result) { sendFileMessage(userKey, result); message.success('文件发送成功'); }
    return false;
  };
  ...
  const renderFileContent = (item: Message) => {
    ...
    return (
      <div className="flex items-center gap-2 p-2 bg-gray-50 rounded cursor-pointer hover:bg-gray-100" onClick={() => item.fileUrl && window.open(item.fileUrl, '_blank')} title="点击打开/下载文件">
    ...
    return (
      <div className="h-80 overflow-y-auto p-3 bg-gray-100 rounded-lg">
        {messages.length === 0 ? (
          ...
          if (item.type === 'ack') return <div key={item.id} className="text-center text-xs text-gray-400 py-1">{item.content}</div>;
          const isMine = item.type === 'sent' || (item.type === 'file' && item.fromUserId === currentUser.id) || (item.fromUserId === currentUser.id);
            ...
            return (
              <div key={item.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} mb-2`}>
                ...
                <div className={`max-w-[70%] ${isMine ? 'order-2' : 'order-1'}`}>
                  ...
                  <div className={`rounded-lg p-2 ${isMine ? 'bg-blue-500 text-white' : 'bg-white shadow-sm'}`}>{renderFileContent(item)}</div>
          ...
          return (
            <div key={item.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} mb-2`}>
              ...
              <div className={`max-w-[70%] ${isMine ? 'order-2' : 'order-1'}`}>
                {!isMine && <div className="text-xs text-gray-500 mb-0.5 ml-1">{displayName}</div>}
                <div className={`rounded-lg px-3 py-2 break-words ${isMine ? 'bg-blue-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none shadow-sm'}`}>{item.content}</div>
                <div className={`text-xs text-gray-400 mt-0.5 ${isMine ? 'text-right' : 'text-left'}`}>{time}</div>
              </div>
              {isMine && <div className="w-8 h-8 rounded-full bg-green-400 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ml-2 order-3">{displayName.charAt(0)}</div>}
            </div>
  ...
  const renderUserPanel = (userKey: UserKey) => {
    ...
    const incomingCall = isA ? incomingCallA : incomingCallB;
    const callType = isA ? callTypeA : callTypeB;
        ...
        <div className="space-y-2">
          ...
          {isConnected && incomingCall ? (
            <div className="flex gap-2 mb-2 p-2 bg-yellow-50 rounded items-center">
              ...
              <Button size="small" type="primary" onClick={() => acceptCall(userKey)}>接听</Button>
              <Button size="small" danger onClick={() => rejectCall(userKey)}>拒接</Button>
            </div>
          ) : isConnected && !inCall ? (
            ...
            <div className="flex gap-2 mb-2 flex-wrap">
              <Button icon={<AudioMutedOutlined />} onClick={() => toggleMute(userKey)}>{muted ? '取消静音' : '静音'}</Button>
              {videoOn && <Button icon={<VideoCameraOutlined />} onClick={() => toggleVideo(userKey)}>关闭视频</Button>}
  ...
  useEffect(() => { return () => { if (wsRefA.current) wsRefA.current.close(); if (wsRefB.current) wsRefB.current.close(); }; }, []);

  return (
    <div className="p-4 max-w-full mx-auto">
      <Card title="💬 WebSocket 双人聊天" className="shadow-lg">
        <div className="flex gap-4 mb-4 items-center">
          <div><Text type="secondary">房间 ID</Text><Input value={roomId} onChange={(e) => setRoomId(e.target.value)} placeholder="room_id" className="w-48" /></div>
          <div className="text-xs text-gray-400">Token: {maskToken(import.meta.env.VITE_ACCESS_TOKEN || '')}</div>
        </div>
